...
setup_ssl_certificate() {
    ...
    if ! command -v ~/.acme.sh/acme.sh &>/dev/null; then
        install_acme
        if [ $? -ne 0 ]; then
            echo -e "${yellow}无法安装 acme.sh，跳过 SSL 设置${plain}"
            return 1
        fi
    fi
    ...
    echo -e "${yellow}注意：必须开放 80 端口以供验证${plain}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确保 80 端口已开放，稍后可运行 x-ui 重试${plain}"
...
setup_ip_certificate() {
    ...
    echo -e "${green}正在设置 Let's Encrypt IP 证书 (短效期)...${plain}"
    echo -e "${yellow}注意：IP 证书有效期约为 6 天，会自动续签。${plain}"
    echo -e "${yellow}默认监听 80 端口。如使用其他端口，请确保外部 80 端口已转发。${plain}"
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        echo -e "${yellow}提示: Let's Encrypt 仍会连接 80 端口，请确保转发配置正确。${plain}"
    fi
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            read -rp "请输入其他端口用于 acme.sh 验证 (留空取消): " alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请检查 ${WebPort} 端口是否可达 (或从外部 80 转发)${plain}"
    ...
    echo -e "${green}正在配置面板证书路径...${plain}"
    ${xui_folder}/x-ui cert -webCert "${certDir}/fullchain.pem" -webCertKey "${certDir}/privkey.pem"
    
    if [ $? -ne 0 ]; then
        echo -e "${yellow}警告: 无法自动设置面板证书路径${plain}"
        echo -e "${yellow}证书位置:${plain}"
    ...
    echo -e "${green}IP 证书安装配置完成！${plain}"
    echo -e "${green}证书有效期约 6 天，acme.sh 会自动续签。${plain}"
    return 0
}
...
ssl_cert_issue() {
    ...
    if [ "${currentCert}" == "${domain}" ]; then
        ...
        echo -e "${red}该域名已存在证书，无法重复签发。${plain}"
        echo -e "${yellow}当前证书详情:${plain}"
        echo "$certInfo"
        return 1
    else
        echo -e "${green}域名准备就绪...${plain}"
    fi
    ...
    local WebPort=80
    read -rp "请选择验证端口 (默认 80): " WebPort
    if [[ ${WebPort} -gt 65535 || ${WebPort} -lt 1 ]]; then
        echo -e "${yellow}输入无效，将使用默认端口 80。${plain}"
        WebPort=80
    fi
    echo -e "${green}将使用端口: ${WebPort} 签发证书。请确保此端口开放。${plain}"

    echo -e "${yellow}正在停止面板...${plain}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${green}签发成功，正在安装...${plain}"
    fi

    reloadCmd="systemctl restart x-ui || rc-service x-ui restart"
    echo -e "${green}默认重载命令 (reloadcmd): ${yellow}systemctl restart x-ui || rc-service x-ui restart${plain}"
    read -rp "是否修改重载命令? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} 预设: systemctl reload nginx ; systemctl restart x-ui"
        echo -e "${green}\t2.${plain} 自定义输入"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${green}证书安装成功，启用自动续签...${plain}"
    fi

    ~/.acme.sh/acme.sh --upgrade --auto-upgrade
    if [ $? -ne 0 ]; then
        echo -e "${yellow}自动续签设置可能有问题，证书详情:${plain}"
        ls -lah /root/cert/${domain}/
        chmod 600 $certPath/privkey.pem 2>/dev/null
        chmod 644 $certPath/fullchain.pem 2>/dev/null
    else
        echo -e "${green}自动续签设置成功${plain}"
        ls -lah /root/cert/${domain}/
        chmod 600 $certPath/privkey.pem 2>/dev/null
        chmod 644 $certPath/fullchain.pem 2>/dev/null
    fi
    ...
    if [[ "$setPanel" == "y" || "$setPanel" == "Y" ]]; then
        ...
        if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then
            ...
            echo -e "${green}已设置面板证书${plain}"
            echo -e "${green}Cert: $webCertFile${plain}"
            echo -e "${green}Key:  $webKeyFile${plain}"
            echo ""
            echo -e "${green}访问地址: https://${domain}:${existing_port}/${existing_webBasePath}${plain}"
            echo -e "${yellow}面板将重启以应用证书...${plain}"
            systemctl restart x-ui 2>/dev/null || rc-service x-ui restart 2>/dev/null
        else
            echo -e "${red}错误: 未找到证书文件: $domain${plain}"
        fi
    else
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}1.${plain} Let's Encrypt 域名证书 (90天，自动续签)"
    echo -e "${green}2.${plain} Let's Encrypt IP 证书 (6天，自动续签)"
    echo -e "${green}3.${plain} 自定义证书路径"
    echo -e "${blue}注:${plain} 选项 1 和 2 需要开放 80 端口。"
    read -rp "请选择 (默认 2 IP证书): " ssl_choice
    ...
    case "$ssl_choice" in
    1)
        echo -e "${green}使用域名证书模式...${plain}"
        ssl_cert_issue
        local cert_domain=$(~/.acme.sh/acme.sh --list 2>/dev/null | tail -1 | awk '{print $1}')
        if [[ -n "${cert_domain}" ]]; then
            SSL_HOST="${cert_domain}"
            echo -e "${green}✓ SSL 证书已配置，域名: ${cert_domain}${plain}"
        else
            echo -e "${yellow}SSL 设置可能已完成，但无法提取域名${plain}"
            SSL_HOST="${server_ip}"
        fi
        ;;
    2)
        echo -e "${green}使用 IP 证书模式...${plain}"
        ...
        if [ $? -eq 0 ]; then
            SSL_HOST="${server_ip}"
            echo -e "${green}✓ IP 证书配置成功${plain}"
        else
            echo -e "${red}✗ IP 证书失败，请检查 80 端口。${plain}"
            SSL_HOST="${server_ip}"
        fi
        ;;
    3)
        echo -e "${green}使用自定义证书...${plain}"
        local custom_cert=""
        local custom_key=""
        local custom_domain=""

        read -rp "请输入证书绑定的域名: " custom_domain
        custom_domain="${custom_domain// /}"

        while true; do
            read -rp "输入证书文件路径 (.crt / fullchain): " custom_cert
            custom_cert=$(echo "$custom_cert" | tr -d '"' | tr -d "'")

            if [[ -f "$custom_cert" && -r "$custom_cert" && -s "$custom_cert" ]]; then
                break
            elif [[ ! -f "$custom_cert" ]]; then
                echo -e "${red}错误: 文件不存在!${plain}"
            elif [[ ! -r "$custom_cert" ]]; then
                echo -e "${red}错误: 文件不可读 (检查权限)!${plain}"
            else
        ...
        while true; do
            read -rp "输入私钥文件路径 (.key / privatekey): " custom_key
        ...
        echo -e "${green}✓ 自定义证书已应用。${plain}"
        echo -e "${yellow}注意: 请自行负责证书续期。${plain}"
...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            read -rp "是否自定义端口? (否则使用随机端口) [y/n]: " config_confirm
            if [[ "${config_confirm}" == "y" || "${config_confirm}" == "Y" ]]; then
                read -rp "请输入端口: " config_port
                echo -e "${yellow}端口设置为: ${config_port}${plain}"
            else
            ...
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${yellow}为保证安全，所有面板必须配置 SSL。${plain}"
            echo -e "${yellow}支持域名和 IP (自动续签)!${plain}"
            echo ""
            ...
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${yellow}⚠ 重要: 请妥善保存凭据!${plain}"
        else
            local config_webBasePath=$(gen_random_string 18)
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "${yellow}检测到默认凭据，正在重置...${plain}"
            ...
            echo -e "${green}用户名、密码和路径配置正常。${plain}"
        fi
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ "$(printf '%s\n' "$min_version" "$tag_version_numeric" | sort -V | head -n1)" != "$min_version" ]]; then
            echo -e "${red}版本过低，请使用 v2.3.5 以上版本。${plain}"
            exit 1
        fi
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载 $1 失败，请确认版本号是否存在${plain}"
            exit 1
        fi
    fi
    ...
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "x-ui/x-ui.db" "/etc/.gitignore"; then
                ...
                echo -e "${green}已添加 x-ui.db 到 .gitignore${plain}"
            fi
        else
            echo "x-ui/x-ui.db" > "/etc/.gitignore"
            echo -e "${green}创建 .gitignore 并忽略 x-ui.db${plain}"
        fi
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            echo -e "${green}发现 x-ui.service，正在安装...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "x-ui.service.debian" ]; then
                        echo -e "${green}发现 x-ui.service.debian，正在安装...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "x-ui.service.arch" ]; then
                        echo -e "${green}发现 x-ui.service.arch，正在安装...${plain}"
                ...
                *)
                    if [ -f "x-ui.service.rhel" ]; then
                        echo -e "${green}发现 x-ui.service.rhel，正在安装...${plain}"
        ...
        if [ "$service_installed" = false ]; then
            echo -e "${yellow}未找到服务文件，正在从 GitHub 下载...${plain}"
            case "${release}" in
                ubuntu | debian | armbian)
            ...
            if [[ $? -ne 0 ]]; then
                echo -e "${red}下载 x-ui.service 失败${plain}"
                exit 1
            fi
            service_installed=true
        fi
        ...
        if [ "$service_installed" = true ]; then
            echo -e "${green}正在配置 systemd...${plain}"
    ...
    echo -e "${green}x-ui ${tag_version}${plain} 安装成功，正在运行..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}x-ui 管理脚本使用说明:${plain}                               │
│                                                       │
